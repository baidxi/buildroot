...
static int reboot_with_arg(const char *arg)
{
	...
	return syscall(__NR_reboot, LINUX_REBOOT_MAGIC1,
			LINUX_REBOOT_MAGIC2, LINUX_REBOOT_CMD_RESTART2, arg);
}
...
static void gui_keymap_load_from_file(struct gui_context_inputs *input)
{
	fprintf(stdout, "%s\n", __func__);
	if (keymap_parse_from_file(input, "/media/boot/keymap.txt") < 0)
		fprintf(stderr, "keymap file parse failed\n");
}
...
struct gui_context * gui_create_context(int argc, char * argv[])
{
	...
	if (context != NULL)
	{
		...
		gui_keymap_load_from_file(&context->inputs);
	...
	while (SDL_PollEvent(&event) != 0)
	{
		...
		switch (event.type)
		{
			...
			case SDL_JOYBUTTONDOWN:
			{
				...
				switch (context->inputs.joy_keymap[event.jbutton.button])
				{
					// The power button
					case POWER_BUTTON:
					{
			...
			case SDL_KEYUP:
			{
				switch(event.key.keysym.sym) {
					case SDLK_POWER:
						context->inputs.internal.current.power = event.key.state;
						break;
					case SDLK_LCTRL:
					case SDLK_RCTRL:
						context->inputs.ctrlkey = event.type = SDL_KEYUP ? true : false;
						break;
					case SDLK_LALT:
					case SDLK_RALT:
						context->inputs.altkey = event.type = SDL_KEYUP ? true : false;
						break;
					case SDLK_r:
						if (context->inputs.ctrlkey &&
							context->inputs.altkey) {
							reboot_with_arg("loader");
						}
						break;
					case SDLK_ESCAPE:
