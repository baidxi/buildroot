...
static void gui_activate_context_menu(struct gui_node * this)
{
	...
	switch (this->parent->type)
	{
		...
		default:
		{
			...
			struct gui_network_menu_node_data *network_menu_node_data = calloc(1, sizeof(*network_menu_node_data));
			
			if (network_menu_node_data) {
				struct gui_node *network_menu_node = gui_create_context_menu_item(this, &first_node, &previous_node, gettext("Network Setup"), gui_activate_network_menu, gui_deactivate_network_menu, gui_update_network_menu, gui_render_vertical_scrolling_list_menu, gui_invalidate_network_menu, NODE_TYPE_NETWORK_CONFIG_MENU, network_menu_node_data);
				if (!network_menu_node)
