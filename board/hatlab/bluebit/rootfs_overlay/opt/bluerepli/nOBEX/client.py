...
import sys
from bluerepli.nOBEX.common import OBEX_Version, OBEXError
from bluerepli.nOBEX.bluez_helper import BluetoothSocket
from bluerepli.nOBEX import headers
from bluerepli.nOBEX import requests
from bluerepli.nOBEX import responses
...
class Client(object):
    ...
    def _send_headers(self, request, header_list, max_length):
        ...
        #print('[DEBUG] client.Client._send_headers: call socket.sendall()')
        self.socket.sendall(request.encode())
        #print('[DEBUG] client.Client._send_headers: returned from socket.sendall()')

        if isinstance(request, requests.Connect):
            #print('[DEBUG] client.Client._send_headers: call decode_connection()')
            response = self.response_handler.decode_connection(self.socket)
            #print('[DEBUG] client.Client._send_headers: returned from decode_connection()')
        else:
            #print('[DEBUG] client.Client._send_headers: call decode()')
            response = self.response_handler.decode(self.socket)
            #print('[DEBUG] client.Client._send_headers: returned from decode()')
    ...
    def connect(self, header_list=()):
        ...
        #print("[DEBUG] OBEX connect call _send_headers()")
        response = self._send_headers(request, header_list, max_length)
        #print("[DEBUG] OBEX connect returned from _send_headers()")
    ...
    def _get(self, name = None, header_list = ()):
        ...
        #print('[DEBUG] Send get request')
        request = requests.Get()
        response = self._send_headers(request, header_list, max_length)
        #print('[DEBUG] Returned from get request')
        yield response
